...
IF "%makeconfig%"=="clean" ( 
	...
	REM original:arm-none-eabi-gcc -o ./bin/%outpath%/led-flash.elf !objects! -mthumb -mcpu=cortex-m3 -TSTM32L152VB_FLASH.ld -nostartfiles -nodefaultlibs -nostdlib -static -Wl,-cref,-u,Reset_Handler -Wl,-Map=./lst/led-flash.map -Wl,--gc-sections -Wl,--defsym=malloc_getpagesize_P=0x80 -Wl,--start-group -lc -lm -Wl,--end-group -specs=nano.specs 

	arm-none-eabi-gcc !objects! -lm -o ./bin/%outpath%/led-flash.elf -mthumb -mcpu=cortex-m3 -TSTM32L152VB_FLASH.ld -Wl,--start-group -lc -lm -Wl,--end-group -static -Wl,-cref,-u,Reset_Handler -Wl,-Map=./lst/led-flash.map -Wl,--gc-sections -Wl,--defsym=malloc_getpagesize_P=0x1000 


	arm-none-eabi-size ./bin/%outpath%/led-flash.elf
